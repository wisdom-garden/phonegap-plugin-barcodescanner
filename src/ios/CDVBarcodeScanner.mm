...
- (UIView*)buildOverlayView {
    ...
    rectArea = CGRectMake(0, 0, rootViewWidth, rootViewHeight - toolbarHeight);
    LBXScanViewStyle *style = [self weixinStyle];
    LBXScanView* reticleView = [[[LBXScanView alloc]initWithFrame:rectArea style:style] autorelease];
    [overlayView addSubview: reticleView];

    [reticleView drawScanRect];
    [reticleView startScanAnimation];
    return overlayView;
}
...
- (LBXScanViewStyle*)weixinStyle
{
    ...
    style.colorAngle = [UIColor colorWithRed:0./255 green:187./255. blue:189./255. alpha:1.0];

    NSBundle *bundle = [NSBundle bundleWithURL:[[NSBundle mainBundle] URLForResource:@"CDVBarcodeScanner" withExtension:@"bundle"]];
    NSString *imagePath = [bundle pathForResource:@"qrcode_scan_weixin_line" ofType:@"png"];
    UIImage *imgLine = [UIImage imageWithContentsOfFile:imagePath];
